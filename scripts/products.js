document.addEventListener('DOMContentLoaded', function() {
    const urlProducts = 'https://6679076c18a459f6394daa0b.mockapi.io/ecommerceScience/products';
    ...
    document.addEventListener('click', function(event) {
        if (event.target && event.target.classList.contains('add-to-cart')) {
            ...
            fetch(`${urlProducts}/${productId}`)
                ...
                .then(product => {
                    var existingItem = cart.find(item => item.product === product.title);
                    if (existingItem) {
                        existingItem.quantity++;
                    } else {
                        cart.push({ product: product.title, price: product.price, quantity: 1 });
                    }
                    updateCart();
